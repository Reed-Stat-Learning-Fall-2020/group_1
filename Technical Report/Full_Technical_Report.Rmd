...
```{r echo=FALSE, fig.height=3, fig.width=5, message=FALSE, fig.cap="**Figure 3.1.** Pitcher wins plotted against future salary; trend line represents linear line of best fit. Correlation between the two values shown underneath."}
#Total Wins
ggplot(Mega_Summary_Filtered,
       aes(
         x = W,
...
```{r echo=FALSE, fig.height=3, fig.width=5, message=FALSE, fig.cap="**Figure 3.2.** Pitcher losses plotted against future salary; trend line represents linear line of best fit. Correlation between the two values shown underneath."}
#Total Losses
ggplot(Mega_Summary_Filtered,
       aes(
         x = L,
...
```{r echo=FALSE, fig.height=3, fig.width=5, message=FALSE, fig.cap="**Figure 3.6.** ERA plotted against future salary; trend line represents linear line of best fit. Correlation between the two values shown underneath."}
#Current ERA
ggplot(Mega_Summary_Filtered,
       aes(
         x = `ERA`,
...
```{r, echo=FALSE, fig.height=3, fig.width=5, message=FALSE, fig.cap="**Figure 7.3.** ERA over hard hit percentage plotted against future salary; trend line represents linear line of best fit. Correlation between the two values shown underneath."}
#ERA/Hard Hit
ggplot(Original_Summary_Filtered,
       aes(
         x = `ERA/Hard Hit %`,
...
```{r, echo=FALSE, fig.height=3, fig.width=5, message=FALSE, fig.cap="**Figure 7.4.** ERA over barrel percentage plotted against future salary; trend line represents linear line of best fit. Correlation between the two values shown underneath."}
#ERA/Barrel
...
pitch_data  %>%
  ...
  theme_classic()
```
Figure 5: Average pitch speed plotted against wOBA. Trend line represents linear line of best fit. Correlation between the two values are shown below.
...
pitch_data  %>%
  ...
  theme_classic()
```
Figure 6: Average pitch speed plotted against wOBA after separating the data into “fast ball” and “off speed”. Trend line represents linear line of best fit. 
...
```
Figure 7: Current ERA plotted against future ERA. Trend line represents linear line of best fit. Correlation between the two values are shown below.
...
ggplot(Original_Summary, aes(x = `ERA (t+1)` ))+
  ...
  theme_classic()
```
Figure 8: Histogram of the distribution of ERA among MLB starting pitchers.
...
ggplot(Original_Summary, aes(x = `ERA (t+1)`, fill = factor(Year))) +
  ...
  theme_classic()
```
Figure 9: Density plot of pitcher ERA separated by year.
...
The ensemble model relied upon combining lasso regression, ridge regression, and random forests into an overall model in the interest of improving model accuracy. Each of the constituent models then is assigned a coefficient to determine the amount they contribute to the final prediction.

For the ERA prediction, the ensemble model's most heavily weighted sub-model was lasso regression with a coefficient of 1.56. This is understandable considering how well the lasso model performed. The ensemble model also included 4 random forests, with weights ranging from 0.28-0.000009.

For the salary prediction, it relied upon only 2 sub-models, a random forest and a lasso regression. The random forest had the higher coefficient, with a weight of 0.98 compared to the lasso regression's 0.29.
...
```
From the above table, we can see that the ensemble model outperformed all other models in forecasting future ERA and salary. This is understandable as combining multiple models allows the ensemble model to "look" at the data in more ways than one which makes it more efficient in terms of utilizing more of the data. 
