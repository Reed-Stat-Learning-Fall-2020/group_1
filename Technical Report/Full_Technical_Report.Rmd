...
In addition to the inferential conclusion, the fact that our best-performing ensemble model’s predictive power greatly exceeded previous null models such as current ERA suggests that it could have broader applications. One obvious application is that similar to the transformations done in the conclusion, one can plot future performance/salary to see which pitchers are under or overcompensated in order to acquire under-valued pitchers. Additionally, one could examine the difference between current ERA and future ERA in order to try to acquire pitchers projected to improve while trading away those predicted to suffer a drop-off. While the predictive power for ERA is still limited, we believe that it can be a valuable accoutrement to decision makers in MLB front offices.

Our most obvious limitation comes is our subpar predictive power for ERA, which is especially evident through the low adjusted R-squared value. Considering the variety of models, we had tested, we do not believe that this is mostly due to statistical modeling but rather due to the high amount of irreducible error or noise. Sport-related statistics inherently suffer from a greater degree of stochasticity and ERA data is no exception. Our models were incredibly hesitant to predict either very high or low ERA seasons, suggesting that such occurrences could be due to chance or are inherently impossible to model.

In summation, in this investigation we used statistical modeling to produce models that resulted in high predictive powers for future salary and more limited accuracy for forecasting future ERA. Consideration of model inputs and weights further suggested that a discrepancy between front office decision making and relatively-new statcast statistics such as xwOBA and xBA haven’t been fully integrated. 
