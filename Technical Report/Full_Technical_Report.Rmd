...
total_quant_pred %>%
  ...
  rearrange() %>%
  shave() %>%
  rplot(shape = 19, colours = c("darkorange", "white", "midnightblue")) +
  theme_classic() +
...
This correlation plot is a great way to quickly look at how some of your predictors are correlated. Right off the bat we can see that we do have a lot of collinearity and although we won't discuss each instance, some salient trends certainly stand out.
...
We also see that, for the most part, this normal distribution is consistent between the different individual years included in the dataset. One somewhat prominent exception is 2019, where `ERA (t+1)` represents a pitcher's ERA in 2020. Due to the inconsistencies and irregularaties in team's preparation for the 2020 season (as a result of COVID-19), batters were abnormally disadvantaged at the beginning of the season. Subsequently, the mean ERA for 2020 is lower than in the four previous years. Unfortunately, this is a difficult effect to seperate from our data. However, as discussed in the methods section, we filterer the data as diligently as possbile (in terms of sample size within individual pitchers' seasons) to ensure as robust a dataset as possible.
