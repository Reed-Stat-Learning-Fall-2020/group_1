...
The main goal of this project was to predict both the future performance and salary of Major League Baseball (MLB) starting pitchers from 2015 to 2019. This endeavor allowed us to not only understand the metrics that underlie the two response variables, but also to investigate if a discrepancy exists between the predictors responsible for prospective pitching success and those responsible for rewarding players with lucrative contracts. A robust data set was constructed from a variety of packages and online sources, all of which are credited throughout the paper. Our analysis and model building were performed in RStudio and, while we applied a number of advanced statistical learning concepts and modeling techniques, we also made use of our preexisting domain knowledge to inform many of our decisions. Ultimately, comparison between a variety of model types revealed that an ensemble model performed the best in forecasting future ERA and salary, outperforming the existing null models. Additionally, inferential conclusions drawn from the more interpretable multiple linear regression models suggested that a non-trivial discrepancy does exist between the contracts handed out by MLB front offices and the predicted future performance of individual starting pitchers. While it seems like front offices are doing an adequate job of considering a variety of measures of pitcher performance, advanced statistics that attempt to account for luck-adjustment have not yet been fully integrated into their decision making.
...
```{r echo=FALSE, fig.height=4, fig.width=6, out.width = "50%"}
...
```{r echo=FALSE, fig.height=3, fig.width=6, message=FALSE, fig.cap="**Figure 9.2.** Average pitch speed plotted against weighted on-base average, separated by pitch category; trend line represents linear line of best fit. Correlation between the two values shown underneath."}
pitch_data  %>%
  ggplot(aes(
    x = `Average Pitch Speed`,
...
We also see that, for the most part, this normal distribution is consistent between the different individual years included in the dataset. One somewhat prominent exception is 2019, where `ERA (t+1)` represents a pitcher's ERA in 2020. Due to the inconsistencies and irregularities in team's preparation for the 2020 season (as a result of COVID-19), batters were abnormally disadvantaged at the beginning of the season. Subsequently, the mean ERA for 2020 is lower than in the four previous years. Unfortunately, this is a difficult effect to mitigate in our data. However, as discussed in the methods section, we filtered the data as diligently as possible (in terms of sample size within individual pitchers' seasons) to ensure as robust a dataset as possible.
...
For our salary forecasting models, we can see that the ensemble model considerably outperformed the other models in terms of both adjusted R-squared and 5-fold MSE. The 5-fold MSE difference might seem negligible, but it must be remembered that the MSE is recorded in `log(Salary)` units. The ridge regression, LASSO regression, custom model, and principal component regression all performed similarly, while the forward model and full model performed the worst. This suggests that, in our analysis of the salary data, in-depth models tend to approximate the data worse than parsimonious models, as the more accurate models generally contained fewer predictors. For our ERA forecasting models, we can see that the forward and custom models produced the greatest adjusted R-squared values. Conversely, in terms of 5-fold MSE, the ensemble model performed the best. However, the 5-fold MSE of the custom model was only marginally worse than that of the ensemble model. When also considering the improvements in adjusted R-squared and coefficient interpretability, the custom model becomes the most favorable in forecasting future ERA---a testament to the strength of manual stepwise regression and the incorporation of domain knowledge.
...
kable(summary(custom_mod)$coef, digits = 3,
      caption = "Custom ERA Model Coefficients") %>%
  kable_styling(bootstrap_options = "striped", font_size = 12, full_width = FALSE)
```

```{r, echo=FALSE}
kable(summary(custom_mod_salary)$coef, digits = 3,
      caption = "Custom Salary Model Coefficients") %>%
  kable_styling(bootstrap_options = "striped", font_size = 12, full_width = FALSE)
```
...
According to our salary model (which had noticeably lower prediction error than our ERA model), most pitchers appeared to be paid at or around their predicted salary, evident in the size of the residuals. That being said, there are still some clear anomalies. Blake Snell, Max Scherzer, Shane Bieber, and Yu Darvish were four of the most underpaid pitchers according to our model. Worth noting is that these four pitchers also had four of the lowest forecasted ERAs. These conclusions illustrate that, when synthesizing the results of the two models, MLB teams may be able to readily identify players who are undervalued, allowing them to make advantageous free agency and contract extension decisions.
...
From both the custom multiple linear regression models and the slightly more confusing LASSO regressions, we can see that a clear disparity does exist between the predictors that inform future performance and those correlated with future salary. The custom salary model did not include a single advanced statistic and still performed relatively well. The correlations examined in the exploratory data analysis further reinforce this trend , suggesting that although front offices are warming up to advanced statistics such as `wOBA`, they are still failing to fully account for luck-adjustment. This is evident in the fact that even though `xwOBA` and `xBA` were both better predictors of future ERA than `wOBA` and `BA`, they were not as highly correlated with future salary. Thus, it seems that a more-holistic adoption of Statcast data could benefit decision making in MLB front offices.

In addition to these inferential conclusions, the fact that the predictive powers of both our ensemble model and custom model greatly exceeded the performance of previous null models suggests that these results could have broader applications. One obvious application is that, similar to the explorations done in the conclusion, front offices can analyze future performance relative to salary to see which pitchers are under or overcompensated in order to acquire undervalued pitchers. Additionally, front offices could examine the difference between current ERA and future ERA in order to try to acquire pitchers projected to improve, while trading away those predicted to suffer a drop-off. While the predictive power for ERA is still limited, we believe that it can be a valuable accoutrement to decision makers in MLB front offices.

Our most obvious limitation comes is our subpar predictive power for ERA, which is especially evident through the low adjusted R-squared value. Considering the extensive variety of models we tested, we do not believe that this effect can be attributed to failures in statistical modeling, but is rather due to the high amount of irreducible error present in the data. Sports-related statistics inherently suffer from a greater degree of stochasticity, and ERA data is no exception. Our models were incredibly hesitant to predict either very high or low ERA seasons, suggesting that such occurrences could be due to chance and are inherently challenging to model.

In summation, in this investigation we used statistical modeling to produce models that resulted in high predictive powers for future salary and more limited accuracy for predicting future ERA. Consideration of model inputs and weights further suggested the prevalence of a sizable discrepancy between front office decision making and forecasted pitcher performance, largely owing to the omission of relatively new but highly predictive Statcast statistics such as `xwOBA` and `xBA` in these decisions. 
